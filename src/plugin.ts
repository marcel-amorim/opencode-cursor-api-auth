...
import {
  ...
  createToolCallsCompletionResponse,
  openAIError,
} from "./plugin/openai.js";
...
import {
  ...
  normalizeCursorAgentModel,
  parseToolCallPlan,
} from "./plugin/tooling.js";
import type { ToolDef } from "./plugin/types.js";
import { createLogger } from "./utils/logger.js";

const log = createLogger("plugin");

type CursorAuthShape = {
  type?: string;
  key?: string;
};
...
interface CursorAgentLaunchResponse {
  id?: string;
}
...
function isChatCompletionsRequest(input: RequestInfo): boolean {
...
function resolveApiKey(auth: Auth | null | undefined): string | undefined {
  if (auth && typeof auth === "object") {
    const typed = auth as CursorAuthShape;
...
function extractApiKeyFromRequestInit(init?: RequestInit): string | undefined {
...
function readGitValue(workspaceDirectory: string, args: string[]): string | undefined {
  try {
...
function hasRemoteBranch(workspaceDirectory: string, ref: string): boolean {
  const value = readGitValue(workspaceDirectory, ["ls-remote", "--heads", "origin", ref]);
  return typeof value === "string" && value.length > 0;
}

function resolveSource(workspaceDirectory: string, sourceRepository?: string, sourceRef?: string): ResolvedSource | null {
...
async function cursorApiRequest<T>(
  ...
  try {
    return { response, data: JSON.parse(text) as T, text };
  } catch {
    return { response, data: null, text };
  }
}

function extractAssistantMessage(conversation: CursorAgentConversationResponse | null): string {
...
async function runCloudAgent(
  ...
  let launch = await cursorApiRequest<CursorAgentLaunchResponse>(apiBaseUrl, apiKey, "/v0/agents", {
    method: "POST",
  ...
  if (
    ...
    const retryLaunchBody: Record<string, unknown> = {
      ...launchBody,
      source,
    };

    launch = await cursorApiRequest<CursorAgentLaunchResponse>(apiBaseUrl, apiKey, "/v0/agents", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  while (true) {
    ...
    const statusResult = await cursorApiRequest<CursorAgentStatusResponse>(apiBaseUrl, apiKey, `/v0/agents/${agentId}`, {
      method: "GET",
    });
    ...
    const conversation = await cursorApiRequest<CursorAgentConversationResponse>(
      apiBaseUrl,
      apiKey,
...
function createStreamingResponse(model: string, output: string, tools: ToolDef[]): Response {
...
function createNonStreamingResponse(model: string, output: string, tools: ToolDef[]): Response {
  const plan = tools.length > 0 ? parseToolCallPlan(output) : null;
  if (plan?.action === "tool_call") {
...
async function parseRequestBody(input: RequestInfo, init?: RequestInit): Promise<unknown> {
      ...
      async loader(getAuth: () => Promise<Auth>) {
        return {
          ...
          fetch: async (input: RequestInfo, init?: RequestInit): Promise<Response> => {
            ...
            try {
              const completion = await runCloudAgent(
                directory,
                config.apiBaseUrl,
                apiKey,
                extracted.model,
